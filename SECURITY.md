...
- The frontend mints one via `POST /api/auth/stream-token` (itself authenticated by the normal non-key chain — forms cookie / Basic+CSRF / `none` / LAN — and **not** key-reachable, since it sits outside `/api/v*`). The token travels as a `?token=` query param because `EventSource` cannot set request headers; it is re-minted transparently to authorize **future opens/reconnects**.
- **Live `/api/events` authorization is connect-time-only, and stream lifetime is bounded (#1796).** A stream is authorized when it opens and is **not** re-checked per event, so a live stream is not torn down mid-flight by token expiry, logout, password change, or secret rotation. To keep a token replayed within its short TTL from yielding an effectively unbounded stream, the server **bounds stream lifetime with a max age** (`MAX_STREAM_AGE_MS`, `event-broadcaster.service.ts`): the heartbeat tick ends any stream past the cap, which triggers the normal `EventSource` error → re-mint → reopen → catch-up path. A dropped stream therefore self-heals rather than silently persisting past the auth-boundary events above. (Graceful shutdown likewise ends all live streams so the process can exit; browsers reconnect to the new process — #1796.)
...
**Additive contract note — discovery-search 409/504 (#2527).** `POST /api/v1/books/:publicId/search` now declares two additional statuses. This is **additive, not a break**: the success shape is byte-for-byte unchanged (`{ data, total }` with opaque `releaseId` values), and neither status is reachable on a request that would previously have returned `200`.

- **`409 SEARCH_IN_PROGRESS`** — reachable only on a *race*. Searches are single-flight per book across every surface (this endpoint, the scheduled and manual library searches, an import-list immediate search, and a failed-download retry). A request that arrives while another of those holds the book is refused outright rather than answered with an empty list, because an empty `{ data: [], total: 0 }` is indistinguishable from a genuine answered zero. No search is started; retry once the in-flight one finishes.
- **`504 SEARCH_TIMEOUT`** — reachable only on a *bound that previously did not exist*. The discovery ladder is now covered by the same shared deadline (`SEARCH_DEADLINE_MS`) as every other search surface, so a run that outlives its budget answers a documented, retryable `504` rather than hanging. It is never surfaced as a `500`, and never as a `200` with an empty `data` array. Note that the expiry bounds how long the *caller* waits, not the work: an immediate retry after a `504` will normally see the `409` above until the abandoned run settles.
