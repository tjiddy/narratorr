...
- **8 frontend test files hardcode `@/lib/api` exports without `...actual` spread**: `ActivityPage.test.tsx`, `SystemInfo.test.tsx`, `SystemSettings.test.tsx`, `BackupTable.test.tsx`, `LibraryPage.test.tsx`, `LibraryPage.url-restore.test.tsx`, `SearchReleasesContent.test.tsx`, `SearchReleasesModal.test.tsx`. Every time a new util is added to the barrel (e.g. `formatBytesPerSec` in #655), these mocks develop dormant "not a function" bugs that only surface when a fixture happens to exercise the new code path. Fix pattern: replace each object-literal mock with `vi.importActual('@/lib/api')` + `...actual` spread + selectively overridden `api.*` methods. Low urgency (currently dormant) but grows with every new barrel export. (discovered in #655)
