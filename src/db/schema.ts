...
export const books = sqliteTable('books', {
  ...
  productionType: text('production_type', {
    ...
    .notNull()
    .default('unknown'),
  // Stable folder suffix for colliding recordings; null preserves single-edition paths.
  // Persist the first deterministic label so later enrichment cannot move the folder.
  editionLabel: text('edition_label'),
  enrichmentAttempts: integer('enrichment_attempts').notNull().default(0),
  // JSON-encoded operator tombstones, intentionally plain text. Drizzle JSON mode parses
  // during every whole-row read, so one corrupt value would break unrelated queries.
  // parseClearedFields degrades invalid data; serializeClearedFields owns canonical writes.
  ...
  // Mirror this hand-managed expression index in 0000_baseline.sql: raw-column
  // uniqueness is case-sensitive, while the partial predicate permits multiple nulls.
...
// CHECK DDL cannot contain bound parameters, so inject literals derived from the TS tuple.
const COMPANION_EBOOK_STATUS_LITERALS = COMPANION_EBOOK_STATUSES.map((s) => `'${s}'`).join(', ');

// book_id must be NOT NULL UNIQUE, not INTEGER PRIMARY KEY: SQLite would otherwise
// generate a rowid for an omitted ID and attach the observation to the wrong book.
// Filesystem times are integer milliseconds; created/updated timestamps are seconds.
// Keep every CHECK total—SQLite accepts a CHECK whose result is NULL.
export const companionEbooks = sqliteTable('companion_ebooks', {
  ...
  /** Narrow to EpubValidationCode at the repository boundary. */
  ...
  // Keep constraint names non-prefixing; rejection tests extract the exact first failure.
  check(
  ...
  // SQLite INTEGER affinity retains fractional REALs; typeof is the storage-class guard.
  check(
  ...
  // A surviving selection may have count 1 after its unselected sibling is deleted.
  // Guard filename explicitly so nullable equality cannot turn the CHECK result into NULL.
  check(
  ...
  // A resolved multi-candidate row must record the owner's selection.
  check(
  ...
  // Filesystem times may be pre-1970; only size is nonnegative.
  check(
...
export const downloads = sqliteTable('downloads', {
  ...
  // clientStatus is remote-client truth; pipelineStage is the local processing overlay.
  // deriveDisplayStatus combines them.
  ...
  // Snapshot before the orchestrator mutates books.status, distinguishing wanted grabs
  // from upgrades at the quality gate. Legacy nulls conservatively mean imported.
...
// Finalized submission headers outlive item pruning; stale receiving headers are GC-eligible.
// Terminal aggregate counts freeze at completion.
export const importSubmissions = sqliteTable('import_submissions', {
  ...
  // Cumulative staged JSON bytes; re-PUT of an existing ordinal adds zero.
  ...
  // Separate indexes serve source-filtered and global newest-first reads; a
  // source-leading index cannot provide global order.
...
// path/title remain projected after payload pruning. bookId identifies an accepted
// placeholder; existingBookId identifies the incumbent for held/skipped rows.
