...
describe('#257 merge observability — BookDetails progress', () => {
  ...
  describe('filesystem path location section', () => {
    let mockExecCommand: ReturnType<typeof vi.fn> | null = null;

    beforeEach(() => {
      // Sibling-describe boundary: #257 block has no `vi.clearAllMocks` in its beforeEach,
      // so toast mocks accumulate calls across tests. Clear them locally.
      vi.clearAllMocks();
    });
    ...
    it('renders the Location section with the monospace path string when libraryBook.path is a non-empty string', () => {
      const path = '/library/audiobooks/Sanderson/The Way of Kings';
      renderBookDetails({ status: 'imported', path });
    ...
    it('does not render the Location section when libraryBook.path is null (status: wanted fixture)', () => {
      renderBookDetails({ status: 'wanted', path: null });
    ...
    it('clipboard success — navigator.clipboard.writeText resolves → writeText called with path, success toast fires', async () => {
    ...
    it('clipboard rejection (no fallback) — writeText rejects → error toast fires and document.execCommand is NOT called', async () => {
      ...
      renderBookDetails({ status: 'imported', path: '/library/book/story.m4b' });
      await user.click(screen.getByTitle('Copy to clipboard'));
    ...
    it('execCommand fallback success — navigator.clipboard undefined + execCommand returns true → success toast fires', async () => {
      const user = userEvent.setup();
      Object.defineProperty(navigator, 'clipboard', {
        get: () => undefined,
        configurable: true,
    ...
    it('execCommand fallback silent failure — navigator.clipboard undefined + execCommand returns false → error toast fires', async () => {
      const user = userEvent.setup();
      Object.defineProperty(navigator, 'clipboard', {
        get: () => undefined,
        configurable: true,
    ...
    it('execCommand fallback throw — navigator.clipboard undefined + execCommand throws → error toast fires', async () => {
      const user = userEvent.setup();
      Object.defineProperty(navigator, 'clipboard', {
        get: () => undefined,
        configurable: true,
    ...
    it('long path renders with break-all and select-all utility classes on the code element', () => {
      ...
      renderBookDetails({ status: 'imported', path: longPath });
