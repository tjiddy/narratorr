...
describe('DownloadActions', () => {
  ...
  describe('approve/reject buttons (pending_review)', () => {
    it('shows Approve button for pending_review status when onApprove provided', () => {
      const download = createMockDownload({ status: 'pending_review' });
      render(<DownloadActions download={download} onApprove={vi.fn()} />);
      expect(screen.getByText('Approve')).toBeInTheDocument();
    });

    it('shows Reject button for pending_review status when onReject provided', () => {
      const download = createMockDownload({ status: 'pending_review' });
      render(<DownloadActions download={download} onReject={vi.fn()} />);
    ...
    it('calls onApprove when Approve button is clicked', async () => {
      ...
      const download = createMockDownload({ status: 'pending_review' });
      render(<DownloadActions download={download} onApprove={onApprove} />);

      await user.click(screen.getByText('Approve').closest('button')!);
    ...
    it('calls onReject when Reject button is clicked', async () => {
      ...
      const download = createMockDownload({ status: 'pending_review' });
      render(<DownloadActions download={download} onReject={onReject} />);

      await user.click(screen.getByText('Reject').closest('button')!);
      expect(onReject).toHaveBeenCalledOnce();
    });

    it('disables Approve button and shows Approving... when isApproving=true', () => {
      const download = createMockDownload({ status: 'pending_review' });
      render(<DownloadActions download={download} onApprove={vi.fn()} isApproving />);

      const btn = screen.getByText('Approving...').closest('button')!;
      expect(btn).toBeDisabled();
    });

    it('disables Reject button and shows Rejecting... when isRejecting=true', () => {
      const download = createMockDownload({ status: 'pending_review' });
      render(<DownloadActions download={download} onReject={vi.fn()} isRejecting />);

      const btn = screen.getByText('Rejecting...').closest('button')!;
      expect(btn).toBeDisabled();
    });
  });
});
