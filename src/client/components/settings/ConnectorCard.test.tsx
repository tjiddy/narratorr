...
describe('ConnectorCard — Plex (registry-driven per-type fields)', () => {
  ...
  it('prunes a fully-blank path-mapping row before validation, reaching onSubmit with pathMappings: [] (#1507)', async () => {
    const onSubmit = vi.fn();
    const user = userEvent.setup();
    renderWithProviders(<ConnectorCard mode="create" onSubmit={onSubmit} onFormTest={vi.fn()} />);
  ...
  it('shows an inline error beside the empty serverPath input for a partial row and does not submit (#1507)', async () => {
    const onSubmit = vi.fn();
    const user = userEvent.setup();
    renderWithProviders(<ConnectorCard mode="create" onSubmit={onSubmit} onFormTest={vi.fn()} />);
    ...
    expect(await screen.findByText('Server path is required')).toBeInTheDocument();
    expect(onSubmit).not.toHaveBeenCalled();
  });

  it('shows an inline error beside the empty localPath input for a partial row and does not submit (#1507)', async () => {
    const onSubmit = vi.fn();
    const user = userEvent.setup();
    renderWithProviders(<ConnectorCard mode="create" onSubmit={onSubmit} onFormTest={vi.fn()} />);
    ...
    expect(await screen.findByText('Local path is required')).toBeInTheDocument();
    expect(onSubmit).not.toHaveBeenCalled();
  });
