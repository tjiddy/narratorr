...
describe('reconcileByClient (F18)', () => {
  it('rejoins on processing', async () => {
    const api = { getSubmissionByClientId: vi.fn(() => Promise.resolve(summary('processing'))) } as never;
  ...
  it('rejoins on complete (poll then drives the one-time detail + surface)', async () => {
    const api = { getSubmissionByClientId: vi.fn(() => Promise.resolve(summary('complete'))) } as never;
  ...
  it('evicts on a receiving header (inert, safely re-runnable)', async () => {
    const api = { getSubmissionByClientId: vi.fn(() => Promise.resolve(summary('receiving'))) } as never;
  ...
  it('treats a 404 as the never-landed evict arm, NOT a lookup failure', async () => {
    const api = { getSubmissionByClientId: vi.fn(() => Promise.reject(new ApiError(404, { error: 'not-found' }))) } as never;
  ...
  it('retries a transient failure then succeeds', async () => {
    ...
    const api = { getSubmissionByClientId } as never;
  ...
  it('exhaustion retains the pointer and reports lookup-failed', async () => {
    const api = { getSubmissionByClientId: vi.fn(() => Promise.reject(new ApiError(503, { error: 'x' }))) } as never;
    const r = await reconcileByClient({ api, clientSubmissionId: 'u', retry: noSleep });
  ...
  it('aborts mid-lookup — discards the result', async () => {
    ...
    controller.abort();
    const api = { getSubmissionByClientId: vi.fn(() => Promise.reject(new DOMException('Aborted', 'AbortError'))) } as never;
