...
describe('reconcileByClient (F18)', () => {
  it('rejoins on processing', async () => {
    const api = { getImportSubmissionByClientId: vi.fn(() => Promise.resolve(summary('processing'))) } as unknown as Pick<Api, 'getImportSubmissionByClientId'>;
  ...
  it('rejoins on complete (poll then drives the one-time detail + surface)', async () => {
    const api = { getImportSubmissionByClientId: vi.fn(() => Promise.resolve(summary('complete'))) } as unknown as Pick<Api, 'getImportSubmissionByClientId'>;
  ...
  it('evicts on a receiving header (inert, safely re-runnable)', async () => {
    const api = { getImportSubmissionByClientId: vi.fn(() => Promise.resolve(summary('receiving'))) } as unknown as Pick<Api, 'getImportSubmissionByClientId'>;
  ...
  it('treats a 404 as the never-landed evict arm, NOT a lookup failure', async () => {
    const api = { getImportSubmissionByClientId: vi.fn(() => Promise.reject(new ApiError(404, { error: 'not-found' }))) } as unknown as Pick<Api, 'getImportSubmissionByClientId'>;
    expect(await reconcileByClient({ api, clientSubmissionId: 'u', retry: noSleep })).toEqual({ action: 'evict', reason: 'never-landed' });
  });

  it('retries a transient failure then succeeds', async () => {
    const getImportSubmissionByClientId = vi
      .fn<() => Promise<SubmissionResponse>>()
      .mockRejectedValueOnce(new ApiError(503, { error: 'x' }))
      .mockResolvedValueOnce(summary('processing'));
    const api = { getImportSubmissionByClientId } as unknown as Pick<Api, 'getImportSubmissionByClientId'>;
    ...
    expect(getImportSubmissionByClientId).toHaveBeenCalledTimes(2);
  });
  ...
  it('exhaustion retains the pointer and reports lookup-failed', async () => {
    const api = { getImportSubmissionByClientId: vi.fn(() => Promise.reject(new ApiError(503, { error: 'x' }))) } as unknown as Pick<Api, 'getImportSubmissionByClientId'>;
    const r = await reconcileByClient({ api, clientSubmissionId: 'u', retry: noSleep });
    expect(r).toEqual({ action: 'lookup-failed' });
    expect(api.getImportSubmissionByClientId).toHaveBeenCalledTimes(5);
  });

  it('aborts mid-lookup — discards the result', async () => {
    const controller = new AbortController();
    controller.abort();
    const api = { getImportSubmissionByClientId: vi.fn(() => Promise.reject(new DOMException('Aborted', 'AbortError'))) } as unknown as Pick<Api, 'getImportSubmissionByClientId'>;
