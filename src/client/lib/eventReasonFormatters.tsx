 ...
 * `QualityComparisonPanel`. Every key is required (a missing key is a hard parse
 * failure routed to the generic fallback); `T | null` fields use `.nullable()` so
 * a present `null` is accepted. The three non-null fields (`action`, `probeFailure`,
 * `holdReasons`) reject `null`/wrong types — the panel reads them unguarded and
 * would render "NaN MB/hr" or throw on `.includes()`/`.length` otherwise. Mirrors
 * `QualityGateData` (src/client/lib/api/activity.ts) / `NULL_REASON`
 * (src/server/services/quality-gate.types.ts). Default `.strip()` tolerates extra
 * keys from other code versions.
 */
