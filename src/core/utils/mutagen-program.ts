 ...
 * Response: { ok, sizeBefore, sizeAfter, verified: { <key>: <read-back string> }, error? }
 *
 ...
 * A requested cover reports its stored byte length under the protocol key `__cover__`, so the
 * caller never needs format-specific knowledge to check it.
 *
...
def run_mp4(request):
    ...
    if cover:
        ...
        audio['covr'] = [MP4Cover(read_cover_bytes(cover), imageformat=image_format)]
    audio.save()
    ...
    if cover:
        ...
        if stored:
            verified['__cover__'] = str(len(bytes(stored[0])))
    return verified
...
def run_id3(request):
    ...
    if cover:
        ...
        if pictures:
            verified['__cover__'] = str(len(pictures[0].data))
    return verified
...
try:
    ...
    if request['format'] == 'mp4':
        verified = run_mp4(request)
    elif request['format'] == 'id3':
        verified = run_id3(request)
    else:
