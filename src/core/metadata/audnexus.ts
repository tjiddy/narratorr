...
// Chapter-runtime lookup (#1932): the `/books/{asin}/chapters` endpoint carries a
...
/**
 * Chapter-runtime lookup outcome (#1932). Preserves the same five kinds as
 * `ProviderLookupResult` so a `rate_limited` survives to the service and can seed
 * shared backoff (F10) — it is NEVER collapsed to an untyped miss. The `ok`
...
/**
 * Reduce a raw chapters record to a usable runtime (#1932). Usable requires:
 ...
 * yields `null` — a valid response with no usable chapter runtime. The single home
 * for the "usable" decision (kept out of the service per the spec's DRY mandate).
 */
...
export class AudnexusProvider implements MetadataEnrichmentProvider {
  ...
  /**
   * Chapter-runtime lookup for the duration-mismatch rescue (#1932). Fetches
   ...
   * `null`) via {@link reduceUsableChapterRuntime}.
   */
