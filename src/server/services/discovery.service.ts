...
export class DiscoveryService {
  ...
  async generateCandidates(signals: LibrarySignals, multipliers?: WeightMultipliers): Promise<ScoredCandidate[]> {
    ...
    // Affinity candidates win ASIN collisions with diversity results.
  ...
  async refreshSuggestions(): Promise<{ added: number; removed: number; warnings: string[] }> {
    ...
    const staleIds = currentPending
      ...
      .map(p => p.id);
    // SQLite permits at most 999 bound parameters.
    ...
    for (const chunk of chunkArray(toUpsert, WRITE_CHUNK_SIZE)) {
      await this.db.insert(suggestions)
        ...
        .onConflictDoUpdate({
          ...
          set: {
            // excluded uses SQLite column names, not Drizzle property names.
  ...
  /**
   * Match case-insensitive ASIN first, then shared pairwise title + primary-author identity.
   * Title matching is non-transitive, so colonBase is only a retrieval bucket; lowest ID wins.
   */
