...
describe('BookService', () => {
  ...
  describe('getAll with search/sort/filter', () => {
    ...
    it('maps downloading status to searching+downloading', async () => {
      ...
      // The where clause should use inArray with ['searching', 'downloading']
    });
    ...
    it('sorts by createdAt ascending when sortDirection=asc', async () => {
      ...
      expect(dataChain.orderBy).toHaveBeenCalledTimes(1);
      // Should have 2 expressions (createdAt + id tiebreaker)
      const args = (dataChain.orderBy as ReturnType<typeof vi.fn>).mock.calls[0];
      expect(args).toHaveLength(2);
    });
  ...
  describe('getStats', () => {
    it('returns tab-model status counts', async () => {
      db.select.mockReturnValueOnce(mockDbChain([
      ...
      // unique authors/series/narrators
      db.select
