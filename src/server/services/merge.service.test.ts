...
describe('#2129 merge_state snapshot', () => {
  ...
  it('reflects and broadcasts every in-flight phase transition', async () => {
    ...
    // …and each phase's discrete merge_progress is immediately followed by its snapshot frame.
    for (const phase of ['staging', 'processing', 'verifying', 'committing']) {
      const index = frames.findIndex((f) => f.event === 'merge_progress' && (f.payload as { phase: string }).phase === phase);
      expect(index).toBeGreaterThanOrEqual(0);
      expect(frames[index + 1]!.event).toBe('merge_state');
      expect((frames[index + 1]!.payload as MergeStateSnapshot).active[0]!.phase).toBe(phase);
    }
  });
  ...
  describe('backstop for the exits that emit no terminal event', () => {
    it('clears the chip when executeMerge finds the book gone', async () => {
      setupBlockingMerge();
      const harness = createSnapshotHarness({ books: [{ id: 42, title: 'Dogs of War' }] });
      ...
      expect(harness.events()).not.toContain('merge_failed');
      expect(harness.service.getMergeStateSnapshot()).toEqual({ active: [], queued: [] });
      // Admission frame + exactly one backstop frame.
      expect(harness.snapshots()).toHaveLength(2);
      expect(harness.snapshots()[1]).toEqual({ active: [], queued: [] });
    });

    it('clears the chip when ffmpeg disappears between validation and execution', async () => {
      setupBlockingMerge();
      const harness = createSnapshotHarness({ books: [{ id: 42, title: 'Dogs of War' }] });
      const row = await harness.bookService.getById(42);
      // executeMerge's own getById is its first await — flip ffmpeg off there so pre-flight
      // still passes and the missing-ffmpeg early return (which emits nothing) is the exit.
      let reads = 0;
