...
function createDeps(overrides: {
  ...
  const log = createLog();
  const retagBook = overrides.retagBook ?? vi.fn().mockResolvedValue({
  ...
  const deps = {
    ...
    log: log as never,
    taggingService: ('taggingService' in overrides ? overrides.taggingService : { retagBook }) as never,
  };

  return { deps, log, retagBook, update, set, where };
}
...
describe('retagMergedOutput — error isolation (#2210 AC13)', () => {
  ...
  it('returns [] without calling retagBook when tagging is disabled', async () => {
    const { deps, retagBook } = createDeps({ taggingEnabled: false });

    expect(await retagMergedOutput(deps, 1, '/library/book/out.m4b')).toEqual([]);
    expect(retagBook).not.toHaveBeenCalled();
  });
  ...
  it('surfaces the retag warnings on a partial failure without throwing', async () => {
    const retagBook = vi.fn().mockResolvedValue({
...
describe('retagMergedOutput — books.size refresh', () => {
  ...
  it('skips the size refresh when nothing was tagged', async () => {
    ...
    const { deps, update } = createDeps({ retagBook });
