...
export async function retagMergedOutput(
  ...
  try {
    ...
    deps.log.warn({ bookId, error: serializeError(error) }, 'Post-merge tag write failed — merge succeeded on disk, but the output carries only its preserved source tags');
    return [];
  }
}
