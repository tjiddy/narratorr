 ...
 * deliberately removed from the undo page. The candidate read, every insert and the marker share
 * one transaction, so a mid-run crash leaves neither a partial ledger nor a set marker — and the
 * marker is therefore also what guarantees the table holds no `added` rows when the inserts run.
 *
