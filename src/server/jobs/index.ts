...
export function startJobs(db: Db, services: Services, log: FastifyBaseLogger) {
  ...
  const jobRegistry: JobEntry[] = [
    ...
    { name: 'search', type: 'timeout', getIntervalMinutes: () => services.settings.get('search').then((s) => s.intervalMinutes), callback: () => runSearchJob(services.settings, services.bookList, services.indexer, services.downloadOrchestrator, log, services.retryBudget, services.eventBroadcaster) },
