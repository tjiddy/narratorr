...
- **Drizzle `$inferSelect` widens enums:** Use actual Zod enum schemas, not `z.string()`, at Drizzle-to-Zod boundaries. For row shapes with enum columns (currently `BookRow` and `DownloadRow`), import the canonical narrowed type from `src/server/services/types.ts` — do NOT redeclare `type FooRow = typeof foos.$inferSelect` per-file (the bare alias re-widens enum columns like `status` / `enrichmentStatus` / `protocol` to `string`).
