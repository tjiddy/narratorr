...
**Search → enrich pipeline trace:** the indexer adapters, blacklist gate, language enrichment, multi-part filter, and quality + language filters emit a per-result audit trail at `debug` level so a single search can be replayed from the log output. Set `LOG_LEVEL=debug` (env var, applied at boot) or set the General settings log level. Grep by `title:` substring (or `guid:` / `infoHash:`) to follow a specific result through the pipeline. Never log apikey, session, mam_id, NZB password tags, FlareSolverr POST bodies, or full response bodies — `serializeError()` redacts URLs from error messages so undici-wrapped failures stay safe.

**Logging errors:** Always wrap `unknown` catch values with `serializeError()` from `src/server/utils/serialize-error.js` before passing to Pino — raw `unknown` values serialize to `{}` in JSON logs. The `narratorr/no-raw-error-logging` ESLint rule enforces this. Use the `error:` key, not `err:` — the lint rule normalizes both, but `error:` is canonical for codebase-wide grep. The rule traces values back to their catch-binding origin: it fires on `{ error: catchBinding.<dot.chain> }` patterns (e.g. `{ error: error.cause }`, `{ error: err.message }`) but does NOT fire on `{ error: serviceResult.error }` where `serviceResult` is a typed result-union — the root identifier matters, not the property name. Computed (`obj[key]`) segments are intentionally skipped. Don't reach for `// eslint-disable` if the rule fires; check whether your value really traces back to a catch binding.

## Debugging

**Folder-name parse tester (`POST /api/library/scan-debug`):** body `{ folderName }` → returns the parsed `{ title, author, series, seriesPosition, asin }`, a per-step `cleanName` trace, the metadata search result, and a library duplicate check. This is the tool for diagnosing import **"No Match"** problems — it shows exactly how a download folder name resolves *before* the metadata lookup, so you can tell a parse failure from a provider miss. The parser is pure functions in `src/server/utils/folder-parsing.ts` (`parseFolderStructure`, `parseFolderStructureRaw`, `cleanNameWithTrace`) with the regex patterns in `folder-parsing-patterns.ts`; to test a name **without auth/HTTP**, import and call them directly via `pnpm exec tsx` (e.g. a throwaway script doing `parseFolderStructure(["Title by Author (Series Book 2)"])` and logging the result).
...
- **Drizzle interactive-prompt hang:** `pnpm db:generate` is non-interactive ONLY for unambiguous diffs (pure adds, drops, new tables). For renames Drizzle emits a multi-choice `select` prompt ("did you rename X→Y, or drop X + add Y?") with no auto-answer flag — in a non-TTY dispatch it hangs the subprocess and a watchdog eventually SIGTERMs the run, losing all in-flight work. Do NOT try `yes`, `echo |`, heredoc, `script`+pty, or `/dev/null` redirection — they all hang. Correct paths: (a) **split the migration** — stage only the drops in `schema.ts`, run db:generate, commit; then stage the adds and re-run; or (b) **hand-write SQL** via `pnpm exec drizzle-kit generate --custom --name <slug>` and write the DDL yourself with `--> statement-breakpoint` separators. Data migrations (UPDATE/DELETE) go in via the same `--custom` path.
- **Drizzle `$inferSelect` widens enums:** Use actual Zod enum schemas, not `z.string()`, at Drizzle-to-Zod boundaries. For row shapes with enum columns (`BookRow`, `DownloadRow`, `IndexerRow`, `DownloadClientRow`, `NotifierRow`, `ImportListRow`, `BookEventRow`, `BlacklistRow`, `SuggestionRow`, `ImportJobRow`), import the canonical narrowed type from `src/server/services/types.ts` — do NOT redeclare `type FooRow = typeof foos.$inferSelect` per-file (the bare alias re-widens enum columns to `string`). A hand-rolled DB-shaped type that types an enum column as `string` (e.g. `type IndexerRow = { ...; type: string; ... }`) is the same anti-pattern in a different syntactic shape — import the canonical type instead.
...
- **Zod `.optional()` rejects null:** Real-world JSON APIs (NYT, Audible, ABS, Hardcover, MAM, Audnexus) commonly return `null` for absent values. `z.string().optional()` accepts `undefined` but **rejects `null`** — schema parses fail with `Expected string, received null`. For ANY field parsed from an external API response, use `.nullish()` (accepts both null and undefined). `.optional()` is fine for fields in our own schemas (request validators, DB-derived shapes, form data, settings) where we control the contract.
...
- **ESM same-module calls bypass `vi.mock`:** When a module's exported function calls another exported function from the same file (e.g. `fetchWithSsrfRedirect` → `fetchWithOptionalDispatcher` inside `network-service.ts`), the inner call goes through the local binding, not the module's exports. A `vi.mock` factory that overrides the inner export will NOT intercept that call — only external callers see the override. Workarounds, in order of preference: (1) mock at the OS boundary (`node:dns/promises`, `node:fs/promises`, `vi.stubGlobal('fetch', ...)`); (2) stub the entry point itself, not its inner dependencies; (3) replace the entry-point implementation entirely in the mock factory (see `cover-download.test.ts:29-67` for the redirect-walker example). Do NOT add `__internal` indirection objects to production code just to enable mocking — that's the dependency-injection-just-for-tests anti-pattern.
- **`vi.clearAllMocks()` does NOT drain `*Once()` queues:** `clearAllMocks` only calls `mockClear` (history); it leaves implementations and `mockResolvedValueOnce` / `mockReturnValueOnce` / `mockImplementationOnce` queues intact. Tests that mix `clearAllMocks()` in `beforeEach` with per-test `*Once()` queueing will leak stale queued responses across tests, producing flaky passes/failures. Use `vi.resetAllMocks()` (or per-mock `mock.mockReset()`) when `*Once()` queues are in play — `resetAllMocks` drains the queue AND restores the implementation.
...
- **Connector refresh queue is best-effort, in-memory:** `ConnectorService`'s debounce/retry refresh queue (`pending` Map of `setTimeout` timers, plus the `draining` in-flight chain) has NO durable backing. `ConnectorService.stop()` drains in-flight flushes on graceful shutdown (wired into `src/server/index.ts`'s `shutdown()` before `app.close()`), and all four queue timers are `unref()`'d so they never pin the event loop past SIGTERM — but a hard crash (SIGKILL/OOM) or a refresh still in its debounce window is dropped by design. The downstream media server (ABS/Plex) reconciles on its own next library change or periodic scan, so a lost refresh is self-healing; do NOT add a durable/DB-backed queue (over-engineering for a single-process self-hosted app — see #769/#877/#885).
