...
Draft a spec for a new issue using the project's spec template, then create it in Gitea.
...
3. Draft the spec body by filling in the template sections:
   ...
   - **Acceptance Criteria**: verifiable behaviors, each mapping to one or more interactions
...
6. Create the issue:
   ```
   node scripts/gitea.mjs issue-create "<title>" --body-file /tmp/spec-body.md "<labels>"
   ```
   Write the spec body to a temp file first, then pass via `--body-file`.
...
- Each acceptance criterion maps to at least one interaction
- Interactions cover error/edge states, not just happy path
