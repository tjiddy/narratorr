...
Picks up a previously blocked issue by finding the blocker context, checking for answers, and restoring the working state. Runs in main context (needs implementation ability).

## Steps

1. **Read the issue:** Run `pnpm gitea issue $ARGUMENTS`. Verify it has `status/blocked` label. If not, STOP: "Issue #<id> is not blocked — nothing to resume."
...
6. **Post resume comment** on the issue:
   - Write to temp file, then: `pnpm gitea issue-comment <id> --body-file <temp-file-path>`
   - Template:
     ```
     **Resuming #<id>**
...
- This skill only resumes — it does NOT implement. After resuming, the user decides next steps (manual implementation or `/implement`)
