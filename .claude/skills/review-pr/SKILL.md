...
1. **Loop guard:** Run `gitea pr-comments <pr-number>`. Count comments containing `## Verdict:` that were posted in the last 24 hours (use the comment timestamps).
   - If **5 or more** verdicts in the last 24 hours → **STOP**: "Loop guard triggered: <N> review verdicts posted on PR #<pr-number> in the last 24 hours. This suggests a review loop. Human intervention needed."
   - If fewer than 5 → proceed.

2. **Fetch PR details:** Run `gitea pr <pr-number>`. Extract:
   - Title, body, state, head branch, base branch, **author** (`author: <login>` in output), labels
   - Linked issue: parse `Refs #<id>` from PR body

3. **Self-review guard:** Run `gitea whoami` to get your authenticated username. Compare against the PR author from step 2.
...
4. **Read linked issue:** Run `gitea issue <id>`. Extract the **Acceptance Criteria** section.

5. **Read project's CLAUDE.md:** Read the `CLAUDE.md` file in the repository root. Extract:
...
6b. **Read prior review history:** Run `gitea pr-comments <pr-number>`. Parse ALL comments containing `## Verdict:` (prior reviews) and `## Review Response` (author responses). Build a map of prior findings and their resolutions:
   ...
   **Dispute engagement rules** — these apply in step 12 when classifying findings. After completing the full review (steps 7-10), check each finding against the prior history map:
...
7. **Check each AC criterion against the diff:**
...
8a. **Behavioral test gap analysis** — For every changed source file in the diff, verify that each new behavior has a corresponding test assertion (not just a test file):
...
8b. **Test quality review** — Go beyond "do tests exist" and evaluate whether they actually catch defects:
...
9. **Architecture review** — Check the diff against the mechanical checks in `.claude/docs/architecture-checks.md`. **Every violation found here MUST become a finding in step 12** — do not note violations in the table without generating corresponding findings.
...
10. **UI/UX design check** — If the issue has `scope/frontend` or the diff includes new/changed UI components (`.tsx` files with JSX):
...
11. **Nitpick filter** — Do NOT flag these as findings:
...
12. **Classify findings** — For **every** issue found in steps 7-10 (including 8b), create a finding entry and assign a `severity` value. Do not cap the number of findings — report everything you find. **Audit yourself**: re-read the Architecture Review table from step 9 and the test quality checks from step 8b. If any row says "violation" or any test quality issue was noted, there MUST be a corresponding finding below. An issue noted in a table but absent from findings is a review defect.
    ...
    **Prior-round deduplication (mandatory for re-reviews):**
    - Before finalizing findings, cross-reference each finding against the prior finding map from step 6b.
    ...
    - If a finding matches one that was previously `disputed`, apply the dispute engagement rules from step 6b. You MUST either withdraw it or raise it with a substantively different reason that rebuts the author's argument. Copy-pasting the old finding is a review defect.
    - Include a `## Prior Findings` section in the review comment (see template in step 13) that explicitly states the disposition of each previously-raised finding: `withdrawn`, `verified-fixed`, or `re-raised (rebuttal: ...)`. This creates an audit trail and forces you to account for every prior finding.

13. **Post review comment on PR (MANDATORY — this is a Gitea API call, not stdout):**
...
14. **Update labels on linked issue (MANDATORY — the orchestrator depends on these transitions):**

    Read the linked issue's current labels from step 4. Then update based on verdict:
...
- **Re-reviews require prior comment reading.** On any PR that already has `## Verdict:` comments, step 6b is mandatory. Skipping it produces review loops where the same finding bounces back and forth. The author has done work to address your findings — respect that by reading their response before re-reviewing.
