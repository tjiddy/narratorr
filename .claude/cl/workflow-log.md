...
## #349 Extract post-import phases from ImportService.importDownload() — 2026-03-16
**Skill path:** /implement → /claim → /plan → /handoff
**Outcome:** success — PR #409

### Metrics
- Files changed: 2 (1 new, 1 modified) | Tests added: 35
- Quality gate runs: 2 (pass on attempt 2 — TagMode type fix)
- Fix iterations: 1 (TagMode string→union type)
- Context compactions: 0

### Workflow experience
- What went smoothly: Clean extraction — existing tests passed immediately after refactoring importDownload() to use extracted functions. The spec's clear separation of hard-fail vs best-effort phases made the extraction boundary obvious.
- Friction / issues encountered: Prior failed implementation attempt left stale plan comments and state that had to be reconciled. Branch was on wrong feature branch (361 vs 349) at conversation start — claim.ts switched it but the shell state drifted. The Explore subagent in coverage review missed the entire import-steps.test.ts file (35 tests) and reported false positives.

### Token efficiency
- Highest-token actions: Reading the full import.service.ts (615 lines) and all issue comments (3 spec review rounds)
- Avoidable waste: The coverage review Explore subagent produced a massive report with 47 "untested" items, most pre-existing in unrelated files. Could be scoped to only branch-changed files.
- Suggestions: Scope coverage subagent to `git diff main --name-only` files only, not transitive dependencies

### Infrastructure gaps
- Repeated workarounds: None
- Missing tooling / config: Coverage review subagent scope is too broad — should only analyze files in the branch diff
- Unresolved debt: importDownload() still needs per-function eslint suppression (complexity 42) — hard-fail pipeline extraction would fix but is out of scope

### Wish I'd Known
1. `TaggingService.tagBook()` uses `TagMode` union type, not string — always check method signatures when extracting callers (see `import-steps-tagmode-type.md`)
2. Settings were fetched 3x mid-import — the snapshot pattern was a real behavior change, not just refactoring
3. The prior failed implement attempt left state (plan comments, claim comments) that made it tricky to know what was already done vs needed fresh
